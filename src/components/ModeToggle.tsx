...
export const ModeToggle: React.FC = () => {
  ...
  return <div className="mb-2 my-[21px]">
      ...
      if (value) setMode(value as "personal" | "business");
        ...
        <ToggleGroupItem value="business" className={cn("flex-1 gap-1.5 h-8 data-[state=on]:bg-background data-[state=on]:shadow", mode === "business" ? "text-sightx-green font-medium" : "text-muted-foreground")}>
...
export const ModeIndicator: React.FC = () => {
  ...
  return <Tooltip>
      <TooltipTrigger asChild>
        <Button variant="ghost" size="icon" className={cn("h-7 w-7 rounded-full", mode === "business" ? "text-sightx-green hover:text-sightx-green/90 hover:bg-sightx-green/10" : "text-sightx-purple hover:text-sightx-purple/90 hover:bg-sightx-purple/10")}>
          {mode === "business" ? <Briefcase className="h-4 w-4" /> : <User className="h-4 w-4" />}
        </Button>
      </TooltipTrigger>
      <TooltipContent>
