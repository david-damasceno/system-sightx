...
const ChatMessage = ({
  ...
  return <div id={`message-container-${message.id}`} className={cn("flex items-start gap-3 group", isAI ? "mr-12" : "ml-12 flex-row-reverse", typing?.isActive ? "animate-pulse" : "animate-fade-in", isHighlighted && "bg-primary/5 py-2 -mx-4 px-4 rounded-lg")}>
      ...
      <div className={cn("relative max-w-[85%] shadow-lg transition-all", isAI ? "message-bubble-ai rounded-2xl rounded-tl-sm" : "message-bubble-user rounded-2xl rounded-tr-sm", typing?.isActive && "animate-pulse")}>
        {/* Message content - Use Markdown for AI messages, plain text for user messages */}
        <div id={`message-${message.id}`}>
          {isAI ? <MarkdownRenderer content={content} isAI={true} /> : <p className="whitespace-pre-wrap">{content}</p>}
        </div>
        
        {/* File attachment */}
        {message.attachment && <div className={cn("mt-3 p-2 rounded-md flex items-center gap-2", isAI ? "bg-white/10" : "bg-black/10")}>
            {message.attachment.type.startsWith("image/") ? <div className="w-full">
                <img src={message.attachment.url} alt={message.attachment.name} className="rounded-md w-full max-h-48 object-contain" />
                <div className="flex justify-between items-center mt-1 px-1 text-xs">
                  <span>{message.attachment.name}</span>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="sm" variant="ghost" className={cn("h-6 w-6 p-0 rounded-full", isAI ? "hover:bg-white/20" : "hover:bg-black/20")} onClick={() => window.open(message.attachment?.url, "_blank")}>
              ...
              </div> : message.attachment.type.startsWith("audio/") ? <div className="w-full">
                <audio controls src={message.attachment.url} className="w-full h-12 rounded my-1">
                ...
                <Button size="sm" variant="ghost" className={cn("h-8 w-8 p-0 rounded-full", isAI ? "hover:bg-white/20" : "hover:bg-black/20")} onClick={() => window.open(message.attachment.url, "_blank")}>
      ...
      {/* Message actions */}
      <MessageActions messageId={message.id} sessionId={message.senderId} isAI={isAI} isFavorite={isFavorite(message.id)} onToggleFavorite={toggleFavorite} onReaction={handleReaction} getUserReaction={getUserReaction} />
    </div>;
};
